-- BRAND INTELLIGENCE COMPLETE SCHEMA
-- Run this in Supabase SQL editor

-- 1. Brand Profile (Fundamentals)
...
CREATE TABLE IF NOT EXISTS brand_financials (
  id BIGSERIAL PRIMARY KEY,
  brand_name TEXT NOT NULL REFERENCES brand_profile(name),
  year INTEGER,
  revenue TEXT,
  market_cap TEXT,
...
-- 3. Brand SKUs (Products)
CREATE TABLE IF NOT EXISTS brand_skus_complete (
  id BIGSERIAL PRIMARY KEY,
  brand_name TEXT NOT NULL REFERENCES brand_profile(name),
  country TEXT,
...
-- 4. Brand Competitors
CREATE TABLE IF NOT EXISTS brand_competitors_complete (
  id BIGSERIAL PRIMARY KEY,
  brand_name TEXT NOT NULL REFERENCES brand_profile(name),
...
-- 5. Competing SKUs
CREATE TABLE IF NOT EXISTS competing_skus_complete (
  id BIGSERIAL PRIMARY KEY,
  brand_name TEXT NOT NULL REFERENCES brand_profile(name),
...
-- 6. White Space / Market Gaps
CREATE TABLE IF NOT EXISTS brand_white_space (
  id BIGSERIAL PRIMARY KEY,
  brand_name TEXT NOT NULL REFERENCES brand_profile(name),
  gap_type TEXT,
  description TEXT,
...
-- 7. Brand Social Media & Presence
CREATE TABLE IF NOT EXISTS brand_social_media (
  id BIGSERIAL PRIMARY KEY,
  brand_name TEXT NOT NULL REFERENCES brand_profile(name),
  platform TEXT,
  followers TEXT,
...
-- 9. Brand Podcasts
CREATE TABLE IF NOT EXISTS brand_podcasts (
  id BIGSERIAL PRIMARY KEY,
  brand_name TEXT NOT NULL REFERENCES brand_profile(name),
  podcast_name TEXT,
  episode_title TEXT,
...
-- 10. Brand AI Strategy
CREATE TABLE IF NOT EXISTS brand_ai_strategy (
  id BIGSERIAL PRIMARY KEY,
  brand_name TEXT NOT NULL REFERENCES brand_profile(name),
  ai_focus_area TEXT,
...
('Nike', 1964, 'Beaverton', 'USA', 'Just Do It', 'American multinational corporation that designs, manufactures, and sells athletic footwear, apparel, and accessories', 'nike.com', 'Beaverton, Oregon, USA')
ON CONFLICT DO NOTHING;

INSERT INTO brand_financials (brand_name, year, revenue, market_cap, profit_margin, growth_rate, net_income, source) VALUES
('Nike', 2024, '$46.7B', '$150B', 11.2, 5.3, '$5.1B', 'Yahoo Finance')
ON CONFLICT DO NOTHING;
...
INSERT INTO brand_social_media (brand_name, platform, followers, reach, estimated_monthly_ad_spend, engagement_rate, investment_score) VALUES
...
CREATE INDEX IF NOT EXISTS idx_brand_financials_name ON brand_financials(brand_name);
CREATE INDEX IF NOT EXISTS idx_brand_skus_name ON brand_skus_complete(brand_name);
CREATE INDEX IF NOT EXISTS idx_brand_competitors_name ON brand_competitors_complete(brand_name);
CREATE INDEX IF NOT EXISTS idx_brand_news_name ON brand_news(brand_name);
CREATE INDEX IF NOT EXISTS idx_brand_podcasts_name ON brand_podcasts(brand_name);
