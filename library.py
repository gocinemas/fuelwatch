...
def create_or_get_pinpoint_collection(company_name: str, brand_key: str) -> str:
    ...
    Returns: Pinpoint collection ID (deterministic hash of brand_key).
    MVP: User manually creates collection in Pinpoint with this ID as name.
    """
    import hashlib
    # Generate deterministic collection ID from brand_key
    # This ensures same company always maps to same collection
    hash_suffix = hashlib.md5(brand_key.encode()).hexdigest()[:8]
    collection_id = f"intel_{brand_key}_{hash_suffix}"
...
def get_pinpoint_collection(brand_key: str) -> dict:
    ...
    return {
        ...
        "pinpoint_url": f"https://journaliststudio.google.com/pinpoint/collections"  # User links manually
    }
