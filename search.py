...
def fetch_brand_data(brand: str, force_refresh: bool = False) -> dict:
    ...
    try:
        ...
        result = {
            ...
            # Agent results from _fetch_wikipedia
            "category_analysis": wiki.get("category_analysis", {}),
            "market_trend": wiki.get("market_trend", {}),
...
"""
    ...
    for b in bindings:
        ...
        try:
            ...
            latest = raw_date
        summary[label] = {"avg": avg, "count": cnt, "latest": latest, "scope": scope_label}
    return summary
...
def fetch_house_prices(postcode: str) -> dict:
    ...
    # Priority 2: Fall back to SPARQL (no database table available)
